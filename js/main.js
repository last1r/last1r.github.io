...
(function renderProjects() {
  ...
  wrap.innerHTML = P.projects.items.map((p) => {
    ...
    const visual = p.visual === "ai" ? neuralSVG() : p.visual === "vision" ? skeletonSVG() : telegramSVG();
    return `
    <article class="project reveal">
      ...
      <div class="project__visual">
        <div class="pv pv--${p.visual}">${visual}<span class="pv-label">SYS_ID: ${esc(p.id.toUpperCase())}</span></div>
      </div>
    </article>`;
  }).join("");
})();
...
(function renderContacts() {
  document.getElementById("contact-grid").innerHTML = P.contact.links.map((l) => `
    <a class="contact-card reveal" href="${esc(l.url)}" ${l.url.startsWith("http") ? 'target="_blank" rel="noopener"' : ""}>
      <span class="contact-card__mono">${esc(l.mono)}</span>
      <span>
        <span class="contact-card__label">${esc(l.label)}</span><br>
...
if (isDesktop && !reduceMotion) {
  ...
  const hot = "a, button, .skill, .project, .contact-card, .stat-card, .wf-step, .tl-item__card, .hub__node";
